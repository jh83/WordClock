...
#define EEPROM_DIMLVL_OFFSET EEPROM_NAME_OFFSET + EEPROM_NAME_LENGTH
#define EEPROM_DIMLVL_LENGTH 1
#define EEPROM_SHOWITIS_OFFSET EEPROM_DIMLVL_OFFSET + EEPROM_DIMLVL_LENGTH
#define EEPROM_SHOWITIS_LENGTH 1
#define EEPROM_LEDTIME_OFFSET EEPROM_SHOWITIS_OFFSET + EEPROM_SHOWITIS_LENGTH
...
class Settings {
  ...
  public:
    ...
    void Load() {
      ...
      showItIs = long(buffer[EEPROM_SHOWITIS_OFFSET]);

      dimLevel = long(buffer[EEPROM_DIMLVL_OFFSET]);

      ledTime = long(buffer[EEPROM_LEDTIME_OFFSET]);
    ...
    void Save() {
      ...
      // Copy dimLevel
      buffer[EEPROM_DIMLVL_OFFSET] = dimLevel;
      // Copy showItIs
      buffer[EEPROM_SHOWITIS_OFFSET] = showItIs;
      // Copy Led On-Time
      buffer[EEPROM_LEDTIME_OFFSET] = ledTime;
